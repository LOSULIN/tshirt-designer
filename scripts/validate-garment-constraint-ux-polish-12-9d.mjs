/**
 * Step 12.9D — Professional Designer Polish 驗證
 * 執行：node scripts/validate-garment-constraint-ux-polish-12-9d.mjs
 */
...
console.log("=== Step 12.9D Professional Designer Polish ===\n");

// Polish module
const polishSrc = readFileSync(
  join(root, "lib/garment-constraint-ux-polish.ts"),
  "utf8",
);
const polishExports = [
  ...
  "getInspectorConstraintBadgeMeta",
  "GARMENT_CONSTRAINT_LEVEL_STYLES",
];
for (const name of polishExports) {
  if (!polishSrc.includes(name)) {
    fail(`garment-constraint-ux-polish 缺少 ${name}`);
  } else {
    pass(`polish API → ${name}`);
  }
}

// DOM / component polish hooks
const domChecks = [
  [
    "CurrentGarmentConstraintVisualization",
    "data-garment-print-constraint-overlay",
  ],
  ["GarmentConstraintUxPrimitives", "data-garment-constraint-layer-badge"],
  ["DesignWorkspaceStatusBar", "data-garment-constraint-print-status"],
  ["GarmentConstraintUxPrimitives", "data-garment-constraint-inspector-badge"],
  ["PlacementPresetToolbar", "data-garment-constraint-warning-level"],
  ["GarmentConstraintUxPrimitives", "data-garment-constraint-warning-level"],
];
for (const [component, attr] of domChecks) {
  const file = `components/designer/${component}.tsx`;
...
for (const anim of [
  "garment-constraint-overlay-pulse",
  "garment-constraint-warning-pulse",
  "garment-constraint-ring-pulse",
]) {
  if (!globalsCss.includes(anim)) {
    fail(`globals.css 缺少 @keyframes ${anim}`);
  } else {
    pass(`animation → ${anim}`);
  }
}
...
for (const file of runtimeFiles) {
  const src = readFileSync(join(root, file), "utf8");
  if (src.includes("garment-constraint-ux-polish")) {
    fail(`${file} 不應 import polish 層`);
  } else {
    pass(`${file} 未修改（無 polish import）`);
  }
}

// 12.9C regression — overlay + constraint geometry
