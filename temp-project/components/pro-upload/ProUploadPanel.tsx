...
import {
  ...
  PRO_UPLOAD_ACCEPT,
  PRO_UPLOAD_FORMATS,
  PRO_UPLOAD_MAX_FILE_SIZE_BYTES,
  PRO_UPLOAD_MAX_FILE_SIZE_MB,
  PRO_UPLOAD_PDF_NOTE,
...
export function ProUploadPanel() {
  ...
  const applyFile = useCallback(async (file: File | undefined) => {
    ...
    if (file.size > PRO_UPLOAD_MAX_FILE_SIZE_BYTES) {
      setSelectedFile(null);
      setInspection(null);
      setIsParsing(false);
      setIsRasterHint(false);
      setGenericHint(`檔案大小超過 ${PRO_UPLOAD_MAX_FILE_SIZE_MB}MB。`);
      return;
    }
