...
const ZOOM_STEPS = [0.75, 0.85, 0.9, 1, 1.1, 1.25] as const;

/** Designer 主畫布：尺碼對應初始 zoom（僅 canvas zoom，不影響模板比例） */
const DEFAULT_CANVAS_ZOOM_BY_SIZE: Record<Size, number> = {
  XS: 1.1,
  S: 1,
  M: 1,
  L: 0.9,
...
function getDefaultZoomIndexForSize(size: Size): number {
  const target = DEFAULT_CANVAS_ZOOM_BY_SIZE[size] ?? 1;
  const index = ZOOM_STEPS.indexOf(target as (typeof ZOOM_STEPS)[number]);
  return index >= 0 ? index : ZOOM_STEPS.indexOf(1);
}
...
export function DesignCanvas({
  ...
  const primaryActionRect = useMemo(() => {
    ...
    if (primaryLayer.type === "text") {
      return getTextLayerPlacementCmRect(primaryLayer);
    }
        ...
        <div className="flex min-h-0 flex-1 overflow-hidden bg-zinc-100">
          {UI_VISIBILITY.showCanvasInfoPanel && (
            <CanvasInfoPanel
              ...
              onKeepRatioChange={onImageKeepRatioChange}
              ...
              >
                {showPrimaryActions && primaryActionRect && primaryLayer && (
                  <LayerFloatingControls
                    ...
                    onScaleDown={() => onLayerToolbarScale(primaryLayer.id, 0.9)}
                    onScaleUp={() => onLayerToolbarScale(primaryLayer.id, 1.1)}
