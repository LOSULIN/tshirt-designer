...
import { getImageFitOptions } from "@/lib/image-print-quality";
import {
...
import {
  ...
  getTextLayerMeasuredCmRect,
  getTextLayerPlacementCmRect,
} from "@/lib/text-layer";
...
import {
  ...
  assessImageUploadQuality,
} from "@/lib/image-processing";
import { validateImageFile } from "@/lib/image-validation";
import { analyzeImageArtworkBoundsFromFile } from "@/lib/image-bounds";
import type {
  ApplicationFormData,
  DesignLayer,
  ImageDesignLayer,
...
export function DesignerApp({
    ...
    (
      ...
      setLayers((prev) =>
        prev.map((layer) => {
          ...
          if (layer.type === "image") {
            const keepRatio = lockAspect;
            const resized = resizeImageLayer(
              layer,
              {
                x_cm: next.x_cm,
                y_cm: next.y_cm,
  ...
  const handleInspectorImageResize = useCallback(
    (
      ...
      const resolvedKeepRatio =
        keepRatio ??
        (layer?.type === "image" || layer?.type === "text"
          ? (layer.keepRatio ?? true)
          : true);
      applyLayerResize(id, next, resolvedKeepRatio);
    },
    [guardEditable, applyLayerResize, layers],
  );

  const handleImageKeepRatioChange = useCallback(
    (id: string, keepRatio: boolean) => {
      if (!guardEditable()) return;
      prepareDiscreteMutation();
      setLayers((prev) =>
        prev.map((layer) => {
          ...
          if (layer.type === "text") {
            if (!keepRatio) {
              ...
              return {
                ...layer,
                keepRatio: false,
                width_cm: measured.width_cm,
                height_cm: measured.height_cm,
              };
            }
    ...
    [guardEditable, prepareDiscreteMutation, setLayers],
  );
  ...
  const handleUpload = async (file: File) => {
    ...
    setIsBusy(true);
    try {
      const quality = assessImageUploadQuality(
        validation.width,
        validation.height,
        validation.isPng,
      );
    ...
    <div className="flex h-full flex-col bg-zinc-50 text-zinc-900">
      <div className="flex min-h-0 flex-1">
        ...
        <DesignCanvas
          ...
          onImageKeepRatioChange={handleImageKeepRatioChange}
