...
export function LayerInspectorEditor({
  ...
  onImageKeepRatioChange?: (id: string, keepRatio: boolean) => void;
  ...
  return (
    <ImageInspectorFields
      ...
      onTransform={(patch) => onImageTransform(layer.id, patch)}
      onResize={(next, keepRatio) =>
        onImageResize(layer.id, next, keepRatio)
      }
      onKeepRatioChange={(keepRatio) =>
        onImageKeepRatioChange?.(layer.id, keepRatio)
      }
      onRotationChange={(rotation) => onRotationChange(layer.id, rotation)}
    />
  );
...
function TextInspectorFields({
  ...
  if (compact) {
    return (
      <div className="space-y-0.5">
        <div className="space-y-1 border-b border-zinc-200 pb-1">
          <p className="text-[10px] font-semibold uppercase tracking-wide text-zinc-500">
            文字資訊
          </p>
          <CompactRow label="文字實際尺寸">
            <span
              className="font-mono font-medium tabular-nums text-zinc-900"
              title={`${formatInspectorDimensionPrecise(bounds.width_cm)} × ${formatInspectorDimensionPrecise(bounds.height_cm)}`}
            >
              {formatInspectorDimensionDisplay(bounds.width_cm)} ×{" "}
              {formatInspectorDimensionDisplay(bounds.height_cm)} cm
            </span>
          </CompactRow>
        </div>
  ...
  return (
    <div className="space-y-2">
      <div className="space-y-1 rounded border border-zinc-200 bg-zinc-50 px-2 py-1.5">
        <p className="text-[10px] font-semibold uppercase tracking-wide text-zinc-500">
          文字資訊
        </p>
...
function ImageInspectorFields({
  ...
  const commitResize = (patch: { width_cm?: number; height_cm?: number }) => {
  ...
  const sizeFields = (
    <ScalableInspectorFields
      ...
      onResize={(next) => {
        ...
        commitResize({
          width_cm:
            Math.abs(next.width_cm - values.width_cm) > 1e-6
              ? next.width_cm
              : undefined,
          height_cm:
            Math.abs(next.height_cm - values.height_cm) > 1e-6
              ? next.height_cm
              : undefined,
        });
      }}
